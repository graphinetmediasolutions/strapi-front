...
const Navbar: React.FC = () => {
  // Example: get locale from URL or context
  const pathname = usePathname();
  const locale = pathname.split("/")[1] || "en";

  const { data, loading, error } = useQuery<NavigationData>(MAIN_MENU_QUERY);

  // const links = data?.mainMenu?.MainMenuItems ?? [];
  ...
  return (
    <nav className="bg-gray-900 py-3 gap-3 flex items-center justify-center px-4">
                  ...
                  <div key={level2.title} className="mb-3">
                    ...
                    <ul className="ml-2 space-y-1 text-sm">
                      {level2.items?.map((level3) => (
                        <li key={level3.title}>
                          <Link
                            href={`/${level3.title.toLowerCase().replace(/\s+/g, "-")}`}
                            passHref
                            legacyBehavior
                          >
